...
- ダイレクトメッセージ（チャット）
- 学習報告の投稿
- 投稿の自動下書き保存（3秒ごと、ブラウザクラッシュ時も復元可能）
- いいね・コメント
...
- 週間・月間アクティビティレポート
- 技術書レビュー共有
- 学習ノート（Notion風ナレッジベース）
  - マークダウン対応のノート作成
  - 階層構造のフォルダ管理（親子関係サポート）
  - タグ・フォルダによる整理
  - お気に入り機能
  - アーカイブ機能（削除せずに非表示・復元可能）
  - ノート複製機能（ワンクリックでコピー作成）
  - ノートテンプレート機能（週報・議事録等の定型フォーマット保存・再利用）
  - ノート間リンク機能（関連ノートの相互参照・バックリンク表示）
  - キーワード検索（タイトル・本文）
...
### Phase 7.1（通知システムの強化）
- [x] 通知設定モデル（NotificationSettings）
- [x] 通知設定のCRUDエンドポイント（取得・更新）
- [x] 通知タイプ別の設定（いいね・コメント・フォロー・メッセージ・メンション）
- [x] 通知配信方法の設定（Webプッシュ・メール・通知音）
- [x] TDDで実装（リポジトリ3テスト、サービス5テスト）
- [x] リアルタイム通知のWebSocket対応
- [x] 未読通知数の取得エンドポイント（既存実装）
- [ ] フロントエンド実装（通知設定UI、未読バッジ、通知音）
- [ ] 全10言語のi18n対応

### Phase 7.2（学習進捗リマインダー）
- [x] リマインダー設定モデル（ReminderSettings）
- [x] リマインダー設定のCRUDエンドポイント（取得・更新）
- [x] 通知頻度設定（毎日・毎週）
- [x] 非アクティブ期間設定（何日間学習がない場合に通知）
- [x] 通知方法設定（Web・メール）
- [x] TDDで実装（リポジトリ3テスト、サービス3テスト）
- [ ] Cron jobで定期実行設定
- [ ] 実際のリマインダー送信ロジック実装
- [ ] フロントエンド実装（リマインダー設定UI）
- [ ] 全10言語のi18n対応

### Phase 7.3（学習ノート — Notion風ナレッジベース）
- [x] Noteモデル＋NoteFolderモデル（マークダウン対応・タグ・フォルダ・お気に入り）
- [x] Repository層: CRUD・検索・お気に入り切り替え（7テストケース）
- [x] Service層: ビジネスロジック（6テストケース）
- [x] Handler層: HTTPエンドポイント（7テストケース）
- [x] `/api/v1/notes` APIエンドポイント（POST/GET/PUT/DELETE/検索/お気に入り）
- [x] 所有者チェック・部分更新対応
- [x] DIコンテナ統合・ルート登録
- [x] TDDで実装（合計20テストケース追加）
