...
func registerPostRoutes(g *gin.RouterGroup, c *di.Container) {
	posts := g.Group("/posts")
	{
		...
		posts.GET("/drafts/count", c.PostHandler.GetDraftsCount)
		...
		posts.GET("/scheduled/count", c.PostHandler.GetScheduledCount)
...
func registerProjectRoutes(g *gin.RouterGroup, c *di.Container) {
	projects := g.Group("/projects")
	{
		...
		projects.GET("/my/count", c.ProjectHandler.GetMyCount)
...
func registerCommunityRoutes(g *gin.RouterGroup, c *di.Container) {
	...
	questions := g.Group("/questions")
	{
		...
		questions.GET("/my/count", c.QuestionHandler.GetMyCount)
	...
	bookReviews := g.Group("/book-reviews")
	{
		...
		bookReviews.GET("/my/count", c.BookReviewHandler.GetMyCount)
