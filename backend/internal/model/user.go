...
type User struct {
	ID                  uint      `json:"id"`
	Username            string    `json:"username"` // 一意のユーザー名（プロフィールURL用）
	...
	GitHubConnected     bool      `json:"github_connected"`  // GitHub連携済みフラグ
	SpotifyConnected    bool      `json:"spotify_connected"` // Spotify連携済みフラグ
	...
	OnboardingCompleted bool      `json:"onboarding_completed"` // 初回セットアップ完了フラグ
	EmailWeeklyReport   bool      `json:"email_weekly_report"`  // ウィークリーレポートメール配信フラグ
	EmailLanguage       string    `json:"email_language"`       // メール配信言語（ja, en, ko等）
