...
type Notification struct {
	ID         uint             `json:"id"`
	UserID     uint             `json:"user_id"` // 通知の受信者
	User       User             `json:"user"`
	Type       NotificationType `json:"type"`     // 通知の種別
	ActorID    uint             `json:"actor_id"` // 通知を発生させたユーザー
	Actor      User             `json:"actor"`
	PostID     *uint            `json:"post_id"` // 関連投稿（該当時のみ）
	Post       *Post            `json:"post,omitempty"`
	QuestionID *uint            `json:"question_id"` // 関連質問（該当時のみ）
	Question   *Question        `json:"question,omitempty"`
	BadgeID    *string          `json:"badge_id,omitempty"` // 関連バッジ（該当時のみ）
