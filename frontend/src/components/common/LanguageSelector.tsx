  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      >
        ...
        <svg className={`w-3 h-3 transition-transform ${isOpen ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
          <path strokeLinecap="round" strokeLinejoin="round" d="M19 9l-7 7-7-7" />
        </svg>
      </button>

      {isOpen && (
        <div className="absolute right-0 mt-1 w-48 bg-gray-800 border border-gray-700 rounded-lg shadow-sm z-50 py-1 max-h-80 overflow-y-auto">
          {languages.map((lang) => (
            <button
              key={lang.code}
